          ...
          <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
            ...
            <div>
              ...
              @if (strtoupper($certificate->division) === 'EXT')
                {{-- Tampilkan read-only untuk eksternal --}}
                <div class="mt-1 block w-full rounded-lg border border-gray-300 bg-gray-100 px-3 py-2 text-gray-900 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100">
                  Eksternal (EXT)
                </div>
                <input type="hidden" name="division" value="EXT">
              @else
                {{-- Tetap dropdown untuk non-EXT --}}
                <select id="division" name="division" required
                  ...
                  @php $currentDivision = old('division', $certificate->division); @endphp

                  {{-- Pastikan value yang tersimpan tetap ada di opsi (jika tidak ada di $divisions) --}}
                  @if($currentDivision && !array_key_exists($currentDivision, $divisions))
                    <option value="{{ $currentDivision }}" selected>{{ $currentDivision }}</option>
                  @endif

                  @foreach($divisions as $code => $label)
                    <option value="{{ $code }}" {{ (string)$currentDivision === (string)$code ? 'selected' : '' }}>
                      {{ $label }} ({{ $code }})
                    </option>
                  @endforeach
                </select>
