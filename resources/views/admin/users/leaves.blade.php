...
<div class="p-6">
  <div class="mb-6">
    <h1 class="text-2xl font-bold text-emerald-800">🗓️ Leave Requests <span class="text-gray-500">/ {{ $user->name ?? ('User #'.$user->id) }}</span></h1>
  ...
  <form method="GET" class="grid md:grid-cols-6 gap-3 bg-white border rounded-xl p-4 shadow-sm mb-6">
    <div class="md:col-span-2">
      ...
      <input type="text" name="q" value="{{ request('q') }}" class="w-full rounded-lg border-gray-300 focus:ring-emerald-500 focus:border-emerald-500" placeholder="Misal: sakit, keluarga, kuliah" />
    </div>
    <div>
    ...
    <div>
      ...
      <input type="text" name="type" value="{{ request('type') }}" class="w-full rounded-lg border-gray-300 focus:ring-emerald-500 focus:border-emerald-500" placeholder="sick / personal / other" />
    </div>
    <div>
      <label class="block text-xs font-medium text-gray-600 mb-1">Status</label>
      <input type="text" name="status" value="{{ request('status') }}" class="w-full rounded-lg border-gray-300 focus:ring-emerald-500 focus:border-emerald-500" placeholder="approved / pending / rejected" />
    </div>
            ...
            <tr class="hover:bg-gray-50">
              ...
              <td class="px-4 py-3">
                ...
                <span class="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium
                  @if($st === 'approved') bg-emerald-100 text-emerald-700
