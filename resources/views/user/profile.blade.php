...
<div class="min-h-screen bg-emerald-300 py-10">
  <div class="max-w-7xl mx-auto p-6 bg-white rounded-3xl shadow-xl ring-1 ring-emerald-200">

    {{-- Flash Messages --}}
    @if(session('success'))
      <div class="mb-4 bg-emerald-50 text-emerald-800 p-3 rounded-xl shadow-sm border border-emerald-200">
        {{ session('success') }}
      </div>
    ...
    <div class="flex items-center gap-6 mb-8">
      <div class="w-16 h-16 sm:w-24 sm:h-24 md:w-32 md:h-32 rounded-full overflow-hidden ring-4 ring-emerald-300 ring-offset-2 ring-offset-white">
        <img
      ...
      <div>
        <h2 class="text-3xl font-semibold text-emerald-900">{{ auth()->user()->name }}</h2>
        <p class="text-zinc-600 text-lg">{{ auth()->user()->email }}</p>

        {{-- Role and Status (aman dari null) --}}
        <div class="mt-3 flex flex-wrap items-center gap-3">
          <span class="inline-flex items-center gap-2 text-xs font-medium px-3 py-1.5 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200">
            <i class="fa-solid fa-user-shield"></i>
            Role: <span class="font-semibold text-emerald-900">{{ auth()->user()->role }}</span>
          </span>
    ...
    <form action="{{ route('user.profile.update') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div class="md:col-span-1">
        ...
        <input
          ...
          class="mt-1 p-3 w-full rounded-lg border border-emerald-200 shadow-sm
      ...
      {{-- Phone Number --}}
      <div class="md:col-span-1">
        <label for="phone_number" class="block text-sm font-medium text-emerald-800">Nomor Telepon</label>
        <input
      ...
      {{-- Password --}}
      <div class="md:col-span-1">
        <label for="password" class="block text-sm font-medium text-emerald-800">Password Baru</label>
        <input
      ...
      <div class="md:col-span-2">
        <label for="profile_picture" class="block text-sm font-medium text-emerald-800">Foto Profil</label>
        <input
        ...
        <div id="image-preview-container" class="mt-4 hidden">
          <img id="image-preview" class="w-32 h-32 object-cover rounded-xl ring-2 ring-emerald-300" alt="Profile Picture Preview">
        </div>
      </div>
      ...
      <div class="md:col-span-2 mt-2 flex justify-end">
        <button
          type="submit"
          class="px-6 py-2.5 bg-emerald-600 text-white rounded-xl shadow
