...
<body class="{{ $whiteBg ? 'bg-white dark:bg-gray-900' : 'bg-gray-50 dark:bg-gray-800' }} overflow-x-hidden">
  ...
  @stack('scripts')
    ...
    <div id="toast" class="fixed top-4 right-4 z-50 w-80 rounded-lg shadow-lg bg-white border border-primary-100 overflow-hidden">
        <div class="px-4 py-3 border-b border-primary-100 font-semibold text-primary-700">
        ...
        <div class="px-4 py-3 text-sm text-gray-700">
            ...
            <ul class="mt-2 list-disc list-inside max-h-40 overflow-auto">
                ...
                <li>
                    ...
                    <span class="font-mono">{{ $row['to'] }}</span>
                </li>
                @endforeach
            </ul>
            @endif
        ...
        <button onclick="document.getElementById('toast')?.remove()" class="absolute top-2 right-2 text-gray-400 hover:text-primary-600">
        ✕
        </button>
    </div>
    @endif
  ...
  <!-- Event Listener for LOA Updates -->
  <script>
